// Pure core of EmailService — split from email.ts (crypto.ts precedent) so
// operator scripts (tsx, where "server-only" doesn't resolve) can exercise
// the REAL send path. App code keeps importing "@/lib/email".
// ---------------------------------------------------------------------------
