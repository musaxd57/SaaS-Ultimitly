...
/**
 * Dry-run the welcome message for upcoming reservations — shows the exact text
 * that would be sent, per guest, WITHOUT sending anything.
 */
export function WelcomeTestButton() {
  const [busy, setBusy] = useState(false);
  const [open, setOpen] = useState(false);
  const [previews, setPreviews] = useState<WelcomePreview[] | null>(null);
  ...
  async function runTest() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/hospitable/welcome-test", { method: "POST" });
      const data = await res.json();
      if (data.ok) {
        setPreviews(data.previews as WelcomePreview[]);
        setOpen(true);
      } else {
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-foreground/40 p-4 sm:p-8">
          <div className="w-full max-w-2xl rounded-xl border border-border bg-card p-5 shadow-xl">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-semibold">Karşılama önizleme</h3>
              <button
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Yaklaşan rezervasyonlara gönderilecek metnin önizlemesi. Hiçbir şey gönderilmedi.
            </p>
            ...
            {missing.length > 0 ? (
              <p className="mt-3 flex items-start gap-2 rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-700">
                ...
                {missing.length} dairede "Karşılama Mesajı" girişi yok ({" "}
                {[...new Set(missing.map((m) => m.property))].join(", ")} ) — bunlara karşılama
                gönderilmez.
              </p>
            ) : null}
            ...
            <div className="mt-3 space-y-3">
              {previews && previews.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  Yaklaşan (45 gün içinde) onaylı rezervasyon bulunamadı.
                </p>
              ) : null}
              {previews
                ?.filter((p) => p.hasEntry)
