...
| **Provenance (V0.4, migration 50): `connectionId` yalnız kanıtla (ingest / gözlemle NULL→X, asla ez), `ingestedAt` = ilk alınma (değişmez), çıkarım backfill YOK, adoption ≠ kanıt, dedupe bağlantı çelişkisi ayrı kova** | **`integration/provenance-ingest`** (Hospitable her iki yön · ilk alınma değişmezliği: değişmeyen VE değişen tekrar senkron · env fallback → bağlan → kaldır · adoption ≠ kanıt / gözlemlenen `observed` / pencere dışı `legacy` · iCal/.csv/QR unbound · enqueue Message damgası), **`unit/provenance-classes`** (kurucu senaryosu: env fallback → sonradan gözlem = `observed`, `ingest` iddiası yok), **`integration/conversation-dedupe-dryrun`** (+5 provenance: `connection_conflict` · NULL↔dolu · zaman farkı ≠ çelişki · `message_connection_conflict` ≠ içerik · tam kopya), `unit/scrub-scope-parity` (kanarya kararı) | ✅ |
| **Messaging capability tek kaynak (V0.5)** | **`unit/messaging-capability`** (fragment == eski literal, doğruluk tablosu, iç thread, automation.ts'te literal yok), **`integration/messaging-capability`** (önizleme == gerçek; iCal/csv/feed/referanssız dışarı; env fallback; bağlantısız org; QR thread), `integration/ical-lifecycle-exclusion` (yapısal 6 spread pini) | ✅ |
| **Ingest sözleşmesi + write service + domain event (V0.6, migration 51)** | **`unit/ingest-adapter-conformance`** (aynı 15 senaryo: ortak fake ↔ gerçek adaptör/fetch stub — normalizasyon · dedupe/sıralama YAPMAZ · durum kümesi · sayfalama · kiracı sınırı (stub varsayımı) · tipli hata · kimliksiz ağa çıkmaz), **`integration/ingest-write-service`** (replay · batch içi tekrar · sırası değişmiş · değişiklik · iptal · kiracı · tombstone (iki katman ölçüldü) · provenance · gövdesiz), mevcut `hospitable-sync` · `sync-conversation-identity-lock` · `sync-p2002-retry-scope` · `erasure` · `reservations-cleanup` değişmeden yeşil, `unit/core-channel-independence` (okuma fonksiyonları yalnız ingest adaptöründe; write service sağlayıcı importu yok) | ✅ — ⚠️ HTTP stub ≠ canlı sağlayıcı doğrulaması |
| **Bağlantı durumu + kimlik çözümü + kuyruk yönlendirmesi (V0.7 kod hazırlığı)** | **`integration/connection-state`** (beş durum × okuma anahtarı kapalı/açık · revoked+env kurucu erişimi · kiracı · resolver forConnectionId kuralları · sağlayıcıya çağrı yok), **`integration/outbox-connection`** (+2: damgalı satır revoked+env → bekler, env ile gitmez, yeniden bağlanınca tek teslim · env altında kuyruklanan satır env ile teslim), V0.3 yarış testleri değişmeden | ✅ |
| **Intelligence V1 — IngestEvent tüketimi, sinyal/hafıza (idempotent · sırasız · yeniden başlatma · iptal/değişiklik · silme-retention · kiracı · KB bootstrap · örüntü)** | **`integration/intelligence-consumer`** (fake ingest → gerçek write service → event → tüketici), **`integration/scheduled-sync-intelligence-hook`**, `unit/core-channel-independence` (intelligence sağlayıcı import etmez) | ✅ — ⚠️ fixture ≠ canlı (prod IngestEvent boş) |
| **V1 ürün akışı — Lixus-native kaynak sözleşmesi (iCal · elle dosya · QR → IngestEvent; KB → hafıza; Hospitable'sız kiracı uçtan uca; KB rota eşitlemesi; yeniden çalıştırma; kiracı; retention)** | **`integration/intelligence-native-sources`** (gerçek `runScheduledSync` + gerçek rotalar; feed HTTP ve model mock), `integration/feed-disappearance` (+1: uzlaştırma iptali event'i), `unit/core-channel-independence` (+1: tek yazıcı pini), `unit/intelligence-labels` | ✅ — ⚠️ fixture ≠ canlı; canlı liste tasarım §6 |
