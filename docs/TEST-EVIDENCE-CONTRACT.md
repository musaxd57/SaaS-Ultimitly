...
| **Provenance (V0.4, migration 50): `connectionId` + `ingestedAt` ingest damgası · bağlantısız ingest ezmez · legacy backfill kapsam/kiracı/idempotent · scheduled-sync kancası** | **`integration/provenance-ingest`** (Hospitable her iki yön · freshness · env fallback → bağlan → kaldır · iCal/.csv/QR yalnız ingestedAt · enqueue Message damgası), **`integration/provenance-backfill`** (kapsam kuralları · ingestedAt uydurulmaz · kiracı · işaret/idempotent · disconnected sahip · ezmeme), **`integration/scheduled-sync-provenance-hook`** (sıra + hata bloklamaz), `unit/scrub-scope-parity` (kanarya kararı) | ✅ |
