...
| **Session revocation (epoch, trusted-device iki-epoch, çıkış)** | `integration/session-epoch`, `trusted-device-login-wiring`, `logout-clears-trusted-device`, `unit/trusted-device`, `require-auth-failclosed` | ✅ |
...
| **OAuth** | `integration/hospitable-oauth`, `hospitable-oauth-context` | ✅ |
...
| **Inbound → sınıflandırma/AI → outbox/send → audit** | `integration/auto-reply-channel` (gate + outbox satırı), `outbox` (worker), `outbox-ops` (audit), `conversations-reply-route`, `escalation-model-path-claim`, `unit/golden-scenarios` (~105 kapı senaryosu) | ⚠️ **Parça parça korunuyor; TEK bir uçtan uca test yok** (inbound → gate → outbox → teslim → audit aynı testte). V0/AI turunda kırmızı-önce eklenecek. |
...
| **Export · deletion · retention · tombstone** | `integration/account-export`, `data-retention`, `retention-message-age-anchor`, `erasure`, `erasure-outbox-revival`, `guest-chat-erasure-gate`, `task-description-scrub`, `unit/scrub-scope-parity` | ✅ — ⚠️ `scrub-scope-parity` DAL düzeyinde kördür; dal-düzeyi pinler ayrı dosyalarda |
