...
## Hospitable OAuth vendor flow kodlandı (2026-07-02, commit b0294da) — DORMANT, endpoint bekliyor
Kullanıcı Partner Portal'a girdi (`partners.hospitable.com`), Public API erişimi onaylandı, "New client" ile
**Client ID + Secret** üretildi → **Railway env'e eklendi** (`HOSPITABLE_OAUTH_CLIENT_ID/SECRET`, repoya asla yazılmadı).
**"Hospitable ile Bağlan" tek-tık OAuth butonu** kodlandı (`lib/hospitable-oauth.ts` + `api/hospitable/oauth/
{authorize,callback}`) — Paddle/Iyzico dormant-adaptör deseniyle: **4 env değişkeninin HEPSİ** set olmadan buton
hiç görünmez, mevcut manuel-token akışı (`hospitable/connect`) koşulsuz çalışmaya devam eder. Callback, alınan
access token'ı **aynı `verifyToken`+`setOrgHospitableToken`** ile saklıyor — OAuth'tan gelen token, manuel
yapıştırılan PAT ile downstream'de (sync/gönderim) birebir aynı davranıyor, yeni bir depolama yolu yok.
Tarayıcıda doğrulandı (Playwright): buton dormant'ta yok, sahte env'le doğru redirect (state cookie + tüm
query param'lar doğru) → sonra env geri alındı. +9 test, 489 yeşil, build temiz.
**⏳ KALAN (canlıya almadan önce):** (1) Gerçek `HOSPITABLE_OAUTH_AUTHORIZE_URL` + `HOSPITABLE_OAUTH_TOKEN_URL` —
`developer.hospitable.com/docs/public-api-docs` OAuth bölümünden veya Patrick'in Notion Partner Resources
linkinden alınacak (kullanıcı arıyor). (2) **Açık soru — token süresi:** OAuth access token süresi doluyor mu,
`refresh_token` veriyor mu? Şu anki kod PAT gibi **süresiz** token varsayıyor; süreliyse token N saat/gün sonra
sessizce geçersiz kalır (sync/gönderim durur) — endpoint netleşince bu da doğrulanıp gerekirse refresh mekanizması
eklenecek. Redirect URI Hospitable'da **`https://www.lixusai.com/api/hospitable/oauth/callback`** olarak kayıtlı.
