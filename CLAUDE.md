...
- **`SHADOW_AI_ENABLED=1`** ✅ (kullanıcı 07-20'de AÇTI + deploy etti) → GLM/Akash gölge sınıflandırıcı Aşama-1
  ...
  **KULLANICI KARARI 07-25: env EKLENMEDİ, olduğu gibi bırakıldı** — gerekçe: pratikte Hospitable'a bağlı tek
  işletme Nuve, gölge yalnız auto_reply gate kararlarında tetiklendiği için bağlantısız yeni kayıt zaten veri
  üretmiyor. Risk BUGÜN sınırlı ama YAPISAL DEĞİL: `orgAllowed` kod-doğrulandı (`if (!raw) return true`), yani
  Hospitable bağlayan İKİNCİ bir müşteri geldiği an onun redakte misafir mesajları da Akash'a akmaya başlar —
  o noktada env'i eklemek ŞART (ya da alt-işleyen metni + DPA hazır olmalı). İkinci ödeyen müşteri = tetikleyici.
