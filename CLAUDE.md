...
- **✅ "TÜM DAİRELERE UYGULA" (kullanıcı UX itirazı — 20 daireye tek tek girmek işkence):** Malzeme Profili formuna
  "Tüm dairelere uygula" butonu (onaylı) + `POST /api/properties/bulk-supply-profile` (withManage, `updateMany` org-scoped
  = IDOR yok, `bulk-times` deseni). Bir daireyi doldur → tüm org dairelerine kopyalar (mevcut profilleri ezer, confirm uyarır).
  +4 test (tümüne uygula · staff 403 · IDOR yok · geçersiz body 400).
- ⏳ Kalan opsiyon: **düşük-stok takibi** = ayrı/daha büyük iş (elde-stok girişi + decrement). Launch-blocker DEĞİL.
