...
## ✅ CODEX SALT-OKUMA DENETİMİ TURU (2026-07-13 — 40 maddelik rapor didik didik; 12 commit, migration YOK)
Kullanıcı Codex'in 40 maddelik salt-okuma denetimini verdi; her madde KODLA doğrulanıp uygulandı/gerekçeli reddedildi/ertelendi.
**Önceki oturumlarda zaten kapatılmıştı:** #2 ENCRYPTION_KEY zorunlu · #3 boot fail-fast (prestart) · #5 strict health (`cbf26f5` — strict'te unknown→503 + no-store + reason) · #10 plan/metin (kullanıcı kararı: kilit YOK, metin gerçeğe `6be5e0d`) · QR saat-kapısı · host-header injection.
**BU TURDA UYGULANAN (hepsi kırmızı-önce/pin testli):**
- **#4 (`c4b4cd5`)** railway.json `healthcheckPath=/api/health` + 300s timeout (yeni container 200 dönmeden trafik almaz; NORMAL probe — strict değil, taze DB'de deploy flap'lemesin) + pin test (startCommand ASLA eklenmesin — Dockerfile migrate+prestart zincirini atlatırdı). ⚠️ "Wait for CI" = Railway paneli (config-as-code değil, OPS).
- **#9 (`3c2e77a`)** `scripts/guard-local-db.mjs`: db:reset/db:push artık loopback-DB şartlı (`--force-reset` seed-guard'dan ÖNCE çalışıyordu = prod-wipe riski); ALLOW_PROD_SEED=1 tek kaçış. `env:recover` gerekçeli KISMEN RED (DB'ye değil working-tree'ye dokunur; konteyner kurtarma hattı).
- **#11 (`c79bc1b`)** email-verify atomik tek-kullanım (updateMany WHERE hash-hâlâ-set; kırmızı kanıt: 8 eşzamanlı tık → 6 session). Index/unique yarısı ERTELENDİ (migration; IP rate-limit zaten var).
- **#13 (`2bd1c18`)** OAuth state cookie artık {state, orgId, userId} taşıyor; callback exchange'den ÖNCE session'la eşitlik şartı → impersonation-çıkışı/org-değişimi ortasında token YANLIŞ tenant'a bağlanamaz (`context_changed`). HMAC gerekmez (eşitlik server-session'a karşı; self-tamper=self-deny).
- **#27 (`7331915`)** landing demo rozeti artık GERÇEK `passesAutoReplySafetyGate` kararı (server-side `wouldAutoSend`); eski 2-kontrol yaklaşımı yüksek-güvenli İADE talebine "gönderilirdi" diyordu. Golden 96/96.
- **#28/#29 (`882df5e`)** `verifyUsedSources` reservation:* alan-whitelist'i (guestName/arrival/departure/status; "reservation:door_code" düşer) + prompt spec'i enumerate edildi. `history`-yarısı GEREKÇELİ RED (prompt sözleşmesi: stil-rehberi olguları da "history"). **statedCheckoutTime artık misafir mesajında DETERMİNİSTİK kanıt şart** (`stated-time.ts timeStatedInMessage` — halüsinasyon rezervasyona yazılamaz; TR/EN saat kalıpları, çıplak sayı guard'ı, \b yerine (?:^|\s) [TR harf dersi]).
- **#30 (`7e64b83`)** translate yapılandırılmış sonuç `{ok}|{ok:false,reason}`; reply route FAIL-CLOSED (çeviri başarısızsa mesaj GÖNDERİLMEZ — eskiden orijinal TR metin misafire "çevrildi" diye gidiyordu); bounded LRU cache (200/6h; sınırsız Map misafir metnini sonsuza tutuyordu) + input 6000/output cap; hatalar cache'lenmez.
- **#7 (`c560cdb`)** hesap-silme foto-temizlik hatası artık reportError'a gider (boş catch yutuyordu); durable-retry S3 işiyle backlog'da.
- **#22 (`89dade9`)** iCal fetch STREAMING byte-cap (chunked/yalancı Content-Length gövdeyi ÖNCE tamamen buffer'lıyordu; sonsuz 1MB-chunk feed ~11 pull'da kesiliyor — testli) + yeni kaynaklarda HTTPS zorunlu (feed URL'leri bearer-benzeri secret taşır; legacy http satırlar sync'te çalışmaya devam). Residual (belgeli): DNS-rebind TOCTOU (pinned-IP dispatcher gerek) + URL at-rest şifreleme (#21, object-storage ile).
- **#36-40 (`4d96ffc`)** landing dürüstlüğü (FAQ opt-in holding-ack'i açıklıyor; "KVKK uyumlu"→"KVKK odaklı tasarım"; "otomatik cevap yok"→"otomatik sonuçlandırmaz") · twitter card summary (görsel yokken large_image bozuk kart) · sitemap /login çıktı + SABİT lastModified (her istekte new Date() yalancı tazelikti; legal=LEGAL_VERSION'dan) · robots TÜM (app) dizinlerini kapsıyor (fs-drift pin testi) + (app) layout noindex (root index:true miras kalıyordu) · DEPLOYMENT.md db-push→migrate-deploy+prestart+healthcheck · .env.example ?secret= yalanı düzeltildi + PRIMARY_ORG_ID prod-fallback-yok.
**GEREKÇELİ RED/RE-YORUM:** #12 rol-değişimi epoch-bump = ürün içinde ROL DEĞİŞTİREN YÜZEY YOK (hiçbir route role yazmıyor; requireSession/requireAuth zaten DB-yetkili) → ölü kod yazılmadı; İLERİDE ekip-yönetimi eklenirse ZORUNLU şart. #28-history = prompt sözleşmesi. #31 52KB prompt kısaltma = RED (prompt-cache aktif, gpt-5.1 ucuz; kısaltma=hot-path rekalibrasyon riski, CLAUDE.md ilkesi).
**ERTELENDİ (kategorili):** [migration] #16 unique'ler (önce prod-dedup) · #17 task-race · #18 Task.origin · #32 RiskEvent · #26 Decimal para · #11-index — hepsi elle doğrulanan ayrı migration turu ister. [infra/OPS] #6 S3/R2+upload-sahiplik · #8 durable outbox (ölçekte) · #19 dağıtık rate-limit · #24 per-org sync kuyruğu · Railway Wait-for-CI + backup/PITR. [ürün kararı] #14 QR per-misafir credential · #15 QR escalation e-postası (e-posta akışı=kullanıcı onayı şart) · #20 2FA recovery · #23 feed-kaybolma reconcile (lastSeenAt tasarımı). [legal/kullanıcı] #1 SELLER · #35 saklama politikası belgesi · #39 gerçek OG görseli/müşteri kanıtı. [düşük] #25 CSV parser rewrite (bilinçli riskli) · #33 yanıt-süresi episode · #34 export kapsam genişletme (veya metin daraltma — karar).

## Durum
**932 test yeşil (bu tur +43), typecheck temiz, next build temiz, migrate deploy canlıda doğrulanmış.** 16 migration
