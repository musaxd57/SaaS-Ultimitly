...
- 07-12/13: boot-gate #3 (prestart; canlı doğrulandı — AUTH_SECRET placeholder outage'ı bunun eksikliğindendi) · ENCRYPTION_KEY zorunlu · host-header injection (appBaseUrl) · QR saat-kapısı · fiyat metni hizası · #5 strict health (`cbf26f5`) · **codex 40-madde salt-okuma turu: 12 commit** (#4 healthcheck `c4b4cd5` · #9 db-guard `3c2e77a` · #11 verify-atomik `c79bc1b` · #13 OAuth-bağlam `2bd1c18` · #27 demo-verdict `7331915` · #28/29 kanıt `882df5e` · #30 translate `7e64b83` · #7 foto `c560cdb` · #22 iCal `89dade9` · #36-40 dürüstlük/SEO/docs `4d96ffc`); gerekçeli RED: #12 (rol-değişim yüzeyi yok) · #28-history (sözleşme) · #31 (prompt kısaltma).

## ⏳ AÇIK İŞLER (tek birleşik liste)
**[migration turu — kullanıcı onayı + prod-dedup ŞART]** `@@unique([conversationId,externalId])` · `(propertyId,sourceReference)` · emailVerifyTokenHash index · Task.origin (system|manual|ai — iptal temizliği manuel görevi de siliyor) · RiskEvent/analitik event modeli (raporlar snapshot sayıyor) · para Decimal/minor-unit.
**[infra/OPS]** S3/R2 foto (upload sahiplik-bağı + signed URL; local /uploads ephemeral) · durable outbox (ölçekte; claim-then-send şimdilik yeterli) · dağıtık rate-limit (Redis/DB; bugün tek-replica) · per-org sync kuyruğu (global kilit tenant'ları seri bağlıyor) · iCal DNS-rebind dispatcher + feed-URL at-rest şifreleme/UI maskeleme · Railway "Wait for CI" + backup/PITR (panel).
**[ürün kararı]** QR per-misafir credential (booking-kanalı PIN) · QR escalation e-postası (e-posta akışı = kullanıcı onayı şartı!) · 2FA recovery kodları · feed-disappearance reconcile (lastSeenAt/missingCount tasarımı) · export kapsamı: ya tamamla ya metni daralt (#34) · opsiyonel liste: haftalık ops-özeti · review-isteme · lost-item · misafir dil/konu analitiği · temizlikçi paylaşım linki.
**[düşük]** CSV tırnak-içi-newline parser (riskli rewrite) · yanıt-süresi episode-bazlı · #46 legalTextHash · OG/Twitter görseli (asset) · account-card error-first→field-first.
**[izle — canlıda doğrula]** AUTO_REPLY ilk gerçek gönderimler + thread'de duplicate yok · safety_emergency false-hold oranı (BİLİNÇLİ AŞIRI-KAPSAYICI: "yangın merdiveni nerede?" bile insana düşer — çok olursa "acil+risk sinyali" kombinasyonuna incelt) · toggle açık org'da smart-task/supply +1 over/under-create · `PADDLE_PLAN_CHANGE_ENABLED=1` ile upgrade/downgrade hesapta doğrula · healthcheck'li ilk deploy'lar (Attempt#→Active) · `isGuestMessage` alan-adını gerçek Hospitable payload'la teyit.
...
## Durum
**932 test yeşil · typecheck temiz · next build temiz · migrate deploy canlıda doğrulanmış.** 17 migration (00_init→16_reservation_calendar_source) sıfır-drift. Branch `claude/great-edison-3zqpZ` origin ile senkron; Railway healthcheck-gated deploy AKTİF. Billing "tamam" (kırmızı-önce kanıtlı). Codex 40-madde salt-okuma denetimi kapatıldı (12 commit + 3 gerekçeli ret); kalan işler yukarıdaki AÇIK İŞLER listesinde — çekirdek defalarca tarandı, yeni tur açmadan önce oradan seç.
