...
- **İddia desteği GÖLGE ölçümü (`ai/claim-support.ts`, 09-23):** cevaptaki saat/tarih/para/kod/telefon/birimli
  ...
  (eskiden `used: []`).
- **Dil:** istem kuralları %100 Türkçe; `input.language` org ayarıdır (algılanan dil değil); `detectGuestLanguage`
  Almancayı İngilizce sanıyor (retrieval'ı etkilemiyor, ölçüldü).
