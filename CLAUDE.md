...
canlı Paddle API anahtarı (Active), client-side token (Active), webhook `https://www.lixusai.com/api/webhooks/paddle`
...
- 💳 **Paddle production CANLI** (env=production, canlı Paddle anahtarı, webhook aktif) → hazır olunca **küçük gerçek ödemeyi birlikte** test et.
...
## Faz-A risk görünürlüğü EKLENDİ (2026-07-03, commit a6b2481, 590 test)
Kullanıcı "bugün ekleyelim mi?" dedi → eklendi. `Conversation.skippedReason` + `lastRiskLevel`
(nullable, migration `4_risk_visibility` throwaway-PG'de sıfır-drift doğrulandı). Yazım noktaları:
model-eskalasyon claim'i reason+risk taşır; keyword şikayeti "complaint" damgalar; low_confidence/
closing_ack/human_hold/reservation_ended korumalı tek-yazım (değer değişmedikçe write yok — 2dk tik
churn'u önlenir); başarılı oto-gönderim reason'ı temizler + verdict'i kaydeder. **UI:** inbox thread'de
kehribar rozet ("AI riskli konu tespit etti — cevap size bırakıldı" / "güven düşük" / "insan devri" /
"konaklama bitti" / "kapanış — cevap gerekmedi"); Raporlar'a "AI Risk Görünümü (son 30 gün)" kartı
