...
**Migration:** 22 klasör `00_init→21_user_verify_hash_index`, zero-pad, sıfır-drift doğrulanmış. Test harness `db push` kullanır → migration-chain drift'i TESTTE YAKALANMAZ; yeni migration'da taze PG'de 00→N `migrate deploy` + `migrate diff --exit-code` ELLE (CI'da migration-chain job'ı da var).
...
**[migration — kalan]** RiskEvent/analitik event modeli (raporlar snapshot sayıyor) · para Decimal/minor-unit · Task `@@unique([propertyId,dedupeKey])` bilinçli DIŞARIDA (semantik: done→aynı-gün yeniden açılabilmeli). ~~conversationId+externalId · propertyId+sourceReference · Subscription provider+providerRef · verify-hash index · Task.origin~~ ✅ m17-21.
...
## Durum
**945 test yeşil · typecheck temiz · next build temiz · migrate deploy canlıda doğrulanmış.** 22 migration (00_init→21_user_verify_hash_index) sıfır-drift (+ dupe-veride reddetme negatif-kanıtlı). Branch `claude/great-edison-3zqpZ` origin ile senkron; Railway healthcheck-gated deploy AKTİF. Billing "tamam" (kırmızı-önce kanıtlı). Codex 40-madde salt-okuma denetimi kapatıldı (12 commit + 3 gerekçeli ret); kalan işler yukarıdaki AÇIK İŞLER listesinde — çekirdek defalarca tarandı, yeni tur açmadan önce oradan seç.
