...
- 07-16: **Codex görsel/UX turu-2 (karar listesi chat'te):** iyzico TAMAMEN KALDIRILDI (rota+client+test — Paddle MoR kilitli, yarım shared-secret auth tuzaktı; geri gelirse GERÇEK imza doğrulamasıyla sıfırdan) · onboarding 6. adım GERÇEK aktivasyon (toggle+premiumAllowed+AUTO_REPLY_ENABLED üçü birden) · Panel "Dikkat gerektirenler" amber bandı (sorunlu konuşma + takılı outbox[failed/review/blocked, manage-rol] + gecikmiş görev + pasif bağlantı[onboarding-sonrası]; temiz günde hiç render olmaz) · sidebar 3 GRUP (Günlük Operasyon / Misafir & İçerik / Yönetim; staff=yalnız Görevler başlıksız) · test harness TEST_DATABASE_URL bypass (Windows/macOS: Linux provisioning atlanır, yalnız db push) · ci.yml Wait-for-CI yorumu iddiasız yapıldı. Gerekçeli RED: zoom 0.95 geri alma (kullanıcı A/B ile seçti; token-bazlı yoğunluk = her sayfada rekalibrasyon) · lint şimdi değil (backlog'a) · onboarding 5. adım "keşfetti" proxy'si kaldı (gerçek ölçüm=analitik gerektirir). Sekmeli Ayarlar SONRAKI TUR (backlog'da, deep-link taşıma şart).
