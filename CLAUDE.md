...
- 07-13 (2): **kontrollü migration turu (m17-21, `6bd46c2`+`aabdf74`):** salt-okuma preflight → prod temizlik (TX-1/TX-2, sayı-assert'li DO blok) → Task.origin + 3 unique + verify-index + hedefli P2002 companion'ları. Kanıtlar: taze 00→21 + zero-drift · NEGATİF kanıt (dupe veride 19 ve 20 REDDETTİ) · dolu-fixture backfill (tuzak satır dahil doğru sınıf) · 945 test. Codex düzeltmeleri uygulandı (kompozit-vs-global gerekçesi, 3 FK çocuk taşıma, hedefli P2002, backfill kesişim önceliği, DO-blok commit güvenliği). Task@@unique gerekçeli DIŞARIDA (done→aynı-gün yeniden-açma semantiği + sync-lock zaten seri).
- 07-13 (4): **para Faz-A turu (m23, `887e2e9`):** Reservation.totalAmount'a DECIMAL(12,2) shadow (`totalAmountDec`) — expand-only, Float DROP yok. Gerçek kapsam: şemadaki 3 Float'tan tek para bu; asıl billing zaten integer-minor (Invoice.amountMinor/priceMinor/Paddle, dokunulmadı); aiConfidence+RiskEvent.confidence Float kaldı. Preflight (prod): 1365/1330 dolu, nan/inf/negatif/yuvarlanacak/asan hepsi 0, EUR 1074+USD 256 → kayıpsız cast. **PG tuzağı: NaN=NaN TRUE** → backfill/healing NOT IN ('NaN','Inf','-Inf') isimle eler (testli; Prisma client NaN satırı okuyamıyor bile). Dual-write 3 yazar (tek-satır=atomik; PATCH yazar değil — şeması yalnız status/notes), okuma Dec-öncelikli tek helper (money.ts), rapor geliri Prisma.Decimal toplamı (0.1+0.2=0.3 kırmızı-önce), Number yalnız display sınırında, RSC-client'a Decimal geçmiyor (denetlendi). **Cutover healing HER sync geçişinde, kilitten hemen sonra, org-döngüsü/Hospitable-402/premium/deep kapılarından ÖNCE** (Codex yakaladı: deep-gate'liydi; 402'li-org senaryosu + narrow-pass testli). Reconciliation sorgusu commit'te; Faz-B şartı: prod'da 0.
- 07-13 (3): **RiskEvent turu (m22, `4dfc935`):** append-only karar geçmişi — YALNIZ nihai deterministik kod kararından sonra yazılır (4 üretim noktası: gate escalated/low-conf/auto-sent + keyword-alerts claim'de), dryRun yazmaz, asla fırlatmaz (persist hatası=reportError, teslimat semantiği değişemez). `@@unique([organizationId,surface,triggerId,finalDecision])` tenant-scoped retry-idempotency (held→sent geçişi yine yazılır) · kapalı-set clamp'ler (riskType 11'lik, reason 4'lük; bilinmeyen=NULL — sanitize DEĞİL, "adalovelace555" dersi) · keyword yolunda riskLevel=NULL (model kararı yok, uydurulmaz) · confidence in-range-or-NULL (clamp yok) · rapor kartı snapshot yerine geçmişten sayar, "yanıtladıklarınız" bilerek Conversation'da, backfill YOK (kart açıklıyor) · propertyId/conversationId FK'sız opak metadata (ChatUsage emsali), org FK CASCADE=KVKK.
- 07-14: **Durable Outbox #8 (m29 çekirdek `23113b6` + entegrasyon `b1ff1f0` + Faz-B `acdc962`, flag `DURABLE_OUTBOX_ENABLED` DEFAULT KAPALI):** mesaj+gönderim-niyeti tek transaction (`enqueueOutbound`), worker FOR UPDATE SKIP LOCKED claim + state machine (pending→sending→sent / ambiguous→reconciling→review / failed) + bounded backoff+seeded-jitter (Math.random YOK → resume+test-deterministik). Codex 7 boşluk kırmızı-önce: **#1** worker flag-KAPALIYKEN-DE drain (`hasDrainableOutbox` — rollback stranded-row bırakmaz) · **#2** per-conv FIFO + tek-ucus (claimBatch NOT EXISTS guard: aynı conv'da eski/uçuşta satır varken sonrakini claim etmez; 2-replika ters/paralel gönderemez; Hospitable 2/dk/rez altında) · **#3** kalıcı UI rozeti (inbox messageOutbox okur; thread Sırada/Gönderiliyor/İletildi/Doğrulanamadı/Gönderilemedi; refresh-sonrası teslim balonu gibi görünmez) · **#4** ÜRETİM reconcile KODDA tutucu (`defaultReconcile`; güvenilir provider-id yok → govde+zaman ile ASLA sent, ambiguous→review; test mock DEĞİL) · **#5** manuel reply + **AI oto-yanıt** bağlandı (güvenlik kapısından SONRA enqueue; senderName GuestOps AI + AI metadata korunur; QR iç HARİÇ) — holding-ack + welcome/checkin/checkout **gerekçeli ERTELENDİ** (migration 30: `markAnsweredOnDelivery` / opsiyonel `conversationId`; proaktif no-Message + sync-dedup kararı; para hot-path'i kullanıcı yokken aceleye getirilmez) · **#6** answered YALNIZ teslimde (enqueue answered YAPMAZ; oluşan outbound Message → sonraki döngü already_answered; idempotencyKey eşzamanlıyı tekilleştirir) · **#7** worker premium/402/org-skip'ten bağımsız + poison izolasyonu. **Bulunmuş gerçek bug:** `settle()` çözüldü — outbox durum-güncellemesi (kaynak-of-truth) Message.externalId link'inden ayrıldı → P2002 link hatası "sent" geçişini geri almaz (yeniden-gönderim kapandı). Kanıt: 1191 test · taze 00→29 zero-drift · flag-OFF yolu birebir. Ağır kalan mimari not: outbox tam-kapsam (5 yol) + durable-outbox-at-scale artık kısmen çözüldü.
...
## Durum
**1191 test yeşil · typecheck temiz · next build temiz · migrate deploy canlıda doğrulanmış.** 30 migration (00_init→29_message_outbox) sıfır-drift taze PG'de doğrulanmış (+ dupe-veride reddetme negatif-kanıtlı). Branch `claude/great-edison-3zqpZ` origin ile senkron; Railway healthcheck-gated deploy AKTİF. Billing "tamam" (kırmızı-önce kanıtlı). Durable Outbox #8 Faz-B kapatıldı (flag DEFAULT KAPALI; manuel+AI oto-yanıt bağlı, holding-ack+lifecycle migration-30'a ertelendi). Kalan işler yukarıdaki AÇIK İŞLER listesinde — çekirdek defalarca tarandı, yeni tur açmadan önce oradan seç.
