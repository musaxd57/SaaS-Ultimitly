...
**iCal:** satırlar kaynağa bağlı (`Reservation.calendarSourceId`; legacy-NULL adoption ATOMİK updateMany) · disappearance-reconcile KAPALI (mass-cancel dersi); STATUS:CANCELLED yalnız KENDİ source satırını iptal eder · fetch: `redirect:"manual"` + 15s timeout + isPrivateHost + fetch-anı `resolvesToPrivate` DNS-check + **streaming 10MB byte-cap** (readBodyCapped) · YENİ kaynak HTTPS zorunlu (legacy http sync'te çalışır) · parser STATUS:CANCELLED okur → yerel iptal + auto-task silinir.
...
**[migration — kalan]** para Faz-B/C (Dec'i tek kaynak yap → Float'ı düşür; ŞART: prod reconciliation=0 teyidi; ~~Faz-A~~ ✅ m23) (~~RiskEvent~~ ✅ m22) · Task `@@unique([propertyId,dedupeKey])` bilinçli DIŞARIDA (semantik: done→aynı-gün yeniden açılabilmeli). ~~conversationId+externalId · propertyId+sourceReference · Subscription provider+providerRef · verify-hash index · Task.origin~~ ✅ m17-21.
**[infra/OPS]** S3/R2 foto (upload sahiplik-bağı + signed URL; local /uploads ephemeral) · durable outbox (ölçekte; claim-then-send şimdilik yeterli) · dağıtık rate-limit (Redis/DB; bugün tek-replica) · per-org sync kuyruğu (global kilit tenant'ları seri bağlıyor) · iCal DNS-rebind dispatcher + feed-URL at-rest şifreleme (UI maskeleme YAPILDI ✓) · Railway "Wait for CI" + backup/PITR (panel).
**[ürün kararı]** QR per-misafir credential (booking-kanalı PIN) · QR escalation e-postası (e-posta akışı = kullanıcı onayı şartı!) · 2FA recovery kodları · feed-disappearance reconcile (lastSeenAt/missingCount tasarımı) · export'u gerçekten TAMAMLAMA (#34'ün metin-daraltma yarısı yapıldı ✓; fatura/audit/foto eklemek istersen ayrı iş) · opsiyonel liste: haftalık ops-özeti · review-isteme · lost-item · misafir dil/konu analitiği · temizlikçi paylaşım linki.
**[düşük]** CSV tırnak-içi-newline parser (riskli rewrite) · yanıt-süresi episode-bazlı · #46 legalTextHash · OG/Twitter görseli (asset).
**[izle — canlıda doğrula]** AUTO_REPLY ilk gerçek gönderimler + thread'de duplicate yok · safety_emergency false-hold oranı (BİLİNÇLİ AŞIRI-KAPSAYICI: "yangın merdiveni nerede?" bile insana düşer — çok olursa "acil+risk sinyali" kombinasyonuna incelt) · toggle açık org'da smart-task/supply +1 over/under-create · healthcheck'li ilk deploy'lar (Attempt#→Active) · `isGuestMessage` alan-adını gerçek Hospitable payload'la teyit.
...
## Durum
**961 test yeşil · typecheck temiz · next build temiz · migrate deploy canlıda doğrulanmış.** 24 migration (00_init→23_reservation_amount_decimal) sıfır-drift (+ dupe-veride reddetme negatif-kanıtlı). Branch `claude/great-edison-3zqpZ` origin ile senkron; Railway healthcheck-gated deploy AKTİF. Billing "tamam" (kırmızı-önce kanıtlı). Codex 40-madde salt-okuma denetimi kapatıldı (12 commit + 3 gerekçeli ret); kalan işler yukarıdaki AÇIK İŞLER listesinde — çekirdek defalarca tarandı, yeni tur açmadan önce oradan seç.
