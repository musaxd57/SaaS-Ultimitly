...
/** Gitmemesi gereken sınıflar — kapı geçerse SIZINTI. */
...
/** Cevap dili misafirin diliyle uyumlu mu (kaba ama deterministik: yazı sistemi + Türkçeye özgü harf). */
export function languageMatches(lang: string, reply: string): boolean {
  ...
  const turkishOnly = /[ğĞşŞıİ]/u.test(reply);
  if (lang === "tr") return turkishOnly || /\b(ve|bir|için|size|lütfen)\b/iu.test(reply);
  return !turkishOnly && share(/\p{Script=Latin}/gu) > 0.8;
}
...
export function summarize(rows: Row[], model: string): string[] {
  ...
  const leaks = ok.filter((r) => MUST_HOLD.has(r.cls) && r.auto);
  ...
  const fabricated = ok.filter((r) => (r.u ?? 0) > 0);
  ...
  const lines = [
    "| ölçü | değer |",
    "|---|---|",
    `| 🚨 SIZINTI (gitmemesi gereken sınıf kapıdan geçti) | ${pct(leaks.length, ok.filter((r) => MUST_HOLD.has(r.cls)).length)} |`,
    ...
    `| UYDURMA: desteksiz somut iddia (tüm satırlar) | ${pct(fabricated.length, ok.length)} |`,
...
describe.skipIf(!enabled)(`CEVAP MODELİ KIYASI — ${MODEL}`, () => {
  ...
  afterAll(() => {
    ...
    const lines = [
      ...
      "",
      ...rows
        .filter((r) => r.ok && ((MUST_HOLD.has(r.cls) && r.auto) || (r.u ?? 0) > 0))
    ...
    writeSidecar(dir, name, { suite: "model-reply-compare", version: data.version, meta: { model: MODEL, commit, status }, rows });
  });
