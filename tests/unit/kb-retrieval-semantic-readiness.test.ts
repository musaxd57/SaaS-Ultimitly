...
const mk = (i: number, over: Partial<{ category: string; title: string; content: string }> = {}) => ({
  ...
  category: "faq",
  title: `Başlık ${i}`,
  // 🚨 KB 30 KALEMİ AŞMAK ZORUNDA (↓`bigKb` 32): küçük KB (≤30 kalem VE ≤24k karakter, 09-23)
  // seçiciyi `small_kb` dalına düşürür ve hiç seçim yapılmaz. Bu dosyanın ölçtüğü her şey
  // seçim yolundadır.
...
const bigKb = (n = 32) => Array.from({ length: n }, (_, i) => mk(i));
