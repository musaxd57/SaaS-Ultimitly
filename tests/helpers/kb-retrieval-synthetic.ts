...
export interface SynQuestion {
  id: string;
  topic: string;
  kind: "tr" | "en" | "syn" | "typo" | "morph" | "guide";
  text: string;
  /** Doğru kaynak kalem kimlikleri (konunun tüm varyantları). */
  goldIds: string[];
  /**
   * CEVAP İÇİN GEREKLİ METİN (Codex 09-09: kimlik değil, cevabı taşıyan cümle):
   * bunlardan biri istem bloğunda geçmiyorsa model doğru cevabı KURAMAZ —
   * kalem kimliği blokta olsa bile (uzun kalemin başka parçası gitmiş olabilir).
   */
  needles: string[];
  /** Rehber soruları için: istem bloğunda bulunması gereken cümle (= needles[0]). */
  mustContain?: string;
}
...
export function makeSyntheticKb(n: number, seed = 42): SyntheticKb {
  ...
  const needleById = new Map<string, string>();
  ...
  for (const [key, ids] of goldByTopic) {
    ...
    const needles = ids.map((id) => needleById.get(id)!);
    for (const kind of ["tr", "en", "syn", "typo", "morph"] as const) {
      questions.push({ id: `q_${key}_${kind}`, topic: key, kind, text: topic.questions[kind], goldIds: ids, needles });
    }
  }
